...
def theorieFunk(x,mu):
    ...
    return np.sin(np.deg2rad(x))-np.cos(np.deg2rad(x))*mu
...
for angle in range(4):
    ...
    for durch in range(3):
        ...
        besch.append(params[0]/g)
        beschErrs.append(np.sqrt(np.diag(errs))[0]/g)
        yAng.append(angles[angle])

print(yAng)
params, serrs = optimize.curve_fit(theorieFunk,yAng,besch)
mures=params[0]
muresErr = np.sqrt(np.diag(serrs))[0]
...
print(muresErr)
i=0
a=ax.errorbar(yAng,besch,fmt=".",yerr=beschErrs, ecolor="black",color="blue",elinewidth=1,capsize=5,capthick=1)
...
c=ax.fill_between(xy[0],xyErr1[1],xyErr2[1],color="orange",alpha=0.4)
ax.legend([a,b,c],[r"Messdaten mit Fehler",r"Fit $sin(\alpha)-\mu_{G}*cos(\alpha)$  mit $\mu_{G}=0.1616(95)$",r"Fehler des Fits"])
