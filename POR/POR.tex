...
\begin{document}
    ...
    In Graph ? ist beispielhaft gezeigt, wie die Messdaten und die gefittete Theoriekurve aussehen.
    Die ausgelesenen Dämfungskonstanten wurden in Graph ? gegen die Dämpfungsstromstärke geplottet und mit der
    Theoriekurve (\ref{dtheo}) gefittet.
    Im Graph ? wurden die ausgelesenen Winkelgeschwindigkeiten gegen die Dämpfung geplottet und mit der Theoriekurve
    (\ref{otheo}) gefittet.
...
n
    ...
    weitere Reibungseffekte, die nicht durch die gedämpft harmonische Schwingung beschrieben können, beachtet werden müssen.
    So gibt es immer Abweichungen bei kleineren Amplituden, die den Fehler des Fits vergrößern. 
